...
function total(arr) {
   var myTotal = arr.reduce(function(a,b){
    return a+b
   },0)
   return myTotal
}
...
function totalVotes(voters){
    return voters.reduce(function(total, number){
...
var watchList = [
  ...
  function calculateRating(arr){
    var chrisFilm =  arr.filter(function(film){
      return film.Director==='Christopher Nolan'
    });
    var totalRate = chrisFilm.reduce(function(total,film){
      return total + parseFloat(film.imdbRating) //  parses a value as a string and returns the first number.
    },0)
    var imdbAverage = totalRate / chrisFilm.length;
    return imdbAverage}
